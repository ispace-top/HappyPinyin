...
| **P1 (Should Have)** | MVP 建议包含，可接受简化版 | 三拼流程（含介母）、拼读过程可视化动画、学习伙伴角色（小熊猫）、拼读成功正向反馈动画 |
| **P2 (Nice to Have)** | MVP 后迭代 | 跟读录音评测、练习/测试模块、积分勋章系统、学习进度追踪、家长/教师模式 |
...
| 选择的音节在轻声下不常见 | 轻声选项始终可用；播放语音后给出提示"轻声通常用于词尾哦～" |
| 浏览器 TTS 对轻声处理不佳 | 轻声时使用无调号文本（如 "ma" 而非 "mā"）作为 TTS 输入，某些引擎能自动处理 |
...
4. **轻声处理深度**: 轻声在初级阶段教学中是否必须？当前设计包含轻声选项，但要简化处理。是否确认 MVP 阶段轻声的处理程度？
