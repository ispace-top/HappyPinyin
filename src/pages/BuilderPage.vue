...
const MEDIAL_NONE = '__none__'

const medialItems = computed<SelectorItem[]>(() =>
  availableMedials.value.map(m => ({
    value: m === null ? MEDIAL_NONE : m,
    label: medialDisplay(m),
    desc: m === null ? '直接拼韵母' : `介母 ${medialDisplay(m)}`,
  }))
)
...
const selectionPreview = computed(() => {
  ...
  if (state.medialChosen && state.selectedMedial) parts.push(medialDisplay(state.selectedMedial))
...
function handleMedialSelect(item: SelectorItem) {
  const medial = item.value === MEDIAL_NONE ? null : item.value
...
// When reaching result step, speak everything in sequence then read word group
...
// For the initial step, use PinyinCard in select mode
...
<template>
  <div class="builder-page">
    ...
    <!-- Step: Medial -->
    <div v-else-if="state.step === 'medial'" class="step-content">
      <button class="back-btn" @click="handleBack">← 返回</button>
      <p class="step-prompt">
        选了声母 <strong>{{ state.selectedInitial }}</strong>，需要加介母吗？
      </p>
      <SelectorGrid
        :items="medialItems"
        :selected-value="state.selectedMedial ?? MEDIAL_NONE"
    ...
    <div v-if="state.step !== 'initial' && state.step !== 'result'" class="selection-preview">
...
@media (min-width: 1024px) {
  .initials-grid { grid-template-columns: repeat(6, 1fr); }
}
</style>
